...
from vmailadmin import application, db
...
def test_accounts(client):
    ...
    rv = client.post("/admin/accounts/new/", data=dict(
        username=username,
        password=password,
        domain=domain,
        enabled="y",
    ...
    rv = client.post("/admin/accounts/delete/", data=dict(
...
def test_aliases(client):
    ...
    goto = random_string()
    rv = client.post("/admin/aliases/new/", data=dict(
        address=address,
        goto=goto,
        active="y",
        url="/admin/aliases/"
    ...
    rv = client.post("/admin/aliases/delete/", data=dict(
        id=address,
        url="/admin/aliases/"
    ), follow_redirects=True)
    assert rv.status_code == 200
...
def test_domains(client):
    ...
    assert rv.status_code == 200
    domain = random_string()
    rv = client.post("/admin/domains/new/", data=dict(
        domain=domain,
        url="/admin/domains/"
    ), follow_redirects=True)
    assert rv.status_code == 200
    assert domain in rv.data.decode("ascii")
    rv = client.post("/admin/domains/delete/", data=dict(
        id=domain,
        url="/admin/aliases/"
    ), follow_redirects=True)
    assert rv.status_code == 200
